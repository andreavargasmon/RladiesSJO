...
=======
## Esta es una traducciÃ³n de este sitio: http://www.datacarpentry.org/R-ecology-lesson/01-intro-to-r.html que tiene 
## licencia CC que permite usarlo, modificarlo pero no sacar provecho comercial.

## inicio de la clase:

##Para hacer comentarios se necesita usar el sÃ­mbolo "#" o seleccionar lo que se quiere poner como 
##comentario y oprimir Ctrl+Shift+c

por ejemplo

##Se pueden obtener resultados con solo poner operaciones matemÃ¡ticas
5*4
9-3
20/2

##TambiÃ©n se pueden asignar valores a objetos
peso_kg<-50
#para asignar valores se puede usar el sÃ­mbolo "<-" o "=", ambos cumplen la misma funciÃ³n

#los objetos pueden tener diferentes nombres, puede ser una letra o una palabra; sin embargo
#no se puede iniciar con un nÃºmero.
2x<-5
x2<-5
...
##EJERCICIO##
#Â¿CÃºal es el valor de los siguientes objetos?
...
#si no se sabe los argumentos de una funciÃ³n se puede usar 
args(round)

#o tambiÃ©n 
?round

##Podemos crear objetos que contengan mÃƒÂ¡s de un valor. Para esto se usan vectores 
##Se definen de la siguiente manera 
peso= c(30,49,55,76)
peso
animales= c("gato", "perro", "gallina")
...
str(peso)  #descripciÃƒÂ³n 
str(animales)

##si necesitamos aÃƒÂ±adir un valor a nuestro vector
peso=c(45,peso)
peso
...
##Si un nuestro vector tenemos valores nulos o faltantes de informaciÃƒÂ³n 
numeros= c(2, 4, 4, NA, 6)
#Si queremos hacer algun cÃƒÂ¡lculo con el vector
mean(numeros) #calcular la media 
...
#Data.frame es la estructura usada para la mayorÃ­a de las estructuras de tabulaciÃ³n de datos. 
#Se usa para obtener estadÃ­sticas y graficar. 

#Se puede generar a mano o utilizar funciones como: 
1.read.csv()
2.read.table()

#Un data.frame es la representaciÃ³n de datos en un formato de tablas donde las columnas son vectores y todos los vectores tienen el mismo largoy el mismo tipo de datos, por ejemplo: 
#Una vez que se tiene un data.frame se puede ver mediante la funciÃ³n str()
...
#AsÃ­ como la funciÃ³n de str, hay otras formas de inspeccionarlo. 
#Para ver el tamaÃ±o o la longitud. 
...
#Me indica el nÃºmero de columnas. 
ncol(planets_df) 
...
#Summary:
#Muestra las estadÃ­sticas de cada columna. 
summary(planets_df)
...
#Se puede convertir un vector no nÃºmerico a factor, Por ejemplo:
sex <- factor(c("male", "female", "female", "male"))
...
#En algunas ocaciones el orden de los elementos dentro de un vector que fun convertido en factor no importa pero en este caso vamos a asignarle nombre a los elementos dentro de Ã©l segÃºn el orden que indica el vector. 
sex <- factor(sex, levels = c("male", "female"))

#TambiÃ©n se puede convertir un factor en caracter, usando as.character(x):
as.character(sex)

#Â¿QuÃ© hacer cuando se tiene factores donde los niveles son nÃºmeros (ejemplo: aÃ±os)?

#La funciÃ³n as.numeric(x) devuelve los valores de Ã­ndice del factor, no sus niveles, 
#por lo que darÃ¡ como resultado un conjunto de nÃºmeros completamente nuevo (y no deseado en este caso). 
...
#Si se quiere observar el nÃºmero de observaciones representadas por cada nivel de factor:
...
###Fechas de formato
#La mejor prÃ¡ctica para tratar con los datos de fecha es garantizar que cada componente de su fecha 
#se almacene como una variable separada. 
